...
std::unordered_map<std::size_t, double> encode(const row_d& labels_d, row_t& labels_t) {
...
void decode(
    const row_t& labels_t, row_d& labels_d, std::unordered_map<std::size_t, double>& leavesMap) {
...
auto main() -> int {
  ...
  for (std::size_t i = 0; i < 100; ++i) {
    std::cout << labels_d[i] << " : " << labels_t[i] << " :: " << prediction_t_to_d[i] << " : "
              << prediction_t[i] << std::endl;
  }
