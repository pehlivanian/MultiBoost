...
int main() {
    ...
    test.start_test("Basic task submission with return values");
    {
        ...
        auto future3 = pool.submit([](const std::string& s) { return s + " World!"; }, std::string("Hello"));
        
        test.assert_true(
            future1.get() == 42 && 
            future2.get() == 30 && 
            future3.get() == "Hello World!",
    ...
    {
        auto future1 = DefaultThreadPool::submitJob_n<2>([]() { 
            return std::this_thread::get_id(); 
        });
        
        auto future2 = DefaultThreadPool::submitJob_n<3>([](int x, int y, int z) { 
            return x + y + z; 
        }, 1, 2, 3);
    ...
    test.start_test("Backward compatibility with runtime parameters");
    {
        ...
        auto future = DefaultThreadPool::submitJob_n(num_threads, []() { 
            return std::string("Backward compatible!"); 
        });
        
        test.assert_true(
            future.get() == "Backward compatible!",
    ...
    test.start_test("Move semantics and perfect forwarding");
    {
        ...
        auto future = pool.submit([](MoveOnlyType obj) {
            return *obj.data * 2;
        }, MoveOnlyType(21));
