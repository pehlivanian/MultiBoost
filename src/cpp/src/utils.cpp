...
std::string writeColMask(const uvec& colMask, boost::filesystem::path fldr) {
  ...
  std::string fileName =
      dumps<ColMaskArchive, CerealIArch, CerealOArch>(cma, SerializedType::COLMASK, fldr);
  return fileName;
}
...
double imbalance(const Row<double>& y) { return imbalance(conv_to<Row<int>>::from(y)); }

double imbalance(const Row<float>& y) { return imbalance(conv_to<Row<int>>::from(y)); }

double err(const Row<double>& yhat, const Row<double>& y, double resolution) {
  if (resolution < 0.) resolution = 100. * std::numeric_limits<double>::epsilon();
...
std::string writeIndex(
    const std::vector<std::string>& fileNames, std::string path, boost::filesystem::path fldr) {
  std::string abs_path;
...
void readIndex(
    std::string path, std::vector<std::string>& fileNames, boost::filesystem::path fldr) {
  std::string abs_path;
...
void mergeIndices(
    std::string pathOld, std::string pathNew, boost::filesystem::path fldr, bool removeOld) {
  ...
  if (removeOld) boost::filesystem::remove(pathOld);
...
std::string fit_prefix(std::size_t depth) {
  ...
  if (depth) {
    pref += "|";
    for (std::size_t i = 0; i < depth; ++i) pref += "____";
    pref += " ";
  }
  return pref;
}
