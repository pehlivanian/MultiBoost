...
void print_scores(std::vector<double>& scores) {
  ...
  std::cout << "[\n";
  std::for_each(
      scores.begin(), scores.end(), [](double score) { std::cout << "[ " << score << "]\n"; });
  std::cout << "]";
}
...
double mixture_of_uniforms(int n) {
  ...
  while (bin < n) {
    if (mixer < static_cast<double>(bin) / static_cast<double>(n)) break;
    ++bin;
  }
