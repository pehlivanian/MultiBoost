...
auto main(int argc, char** argv) -> int {
  ...
  const auto
      [error_OOS,
       ...
       F1_IS,
       imbalance_IS] =
          Replay<DataType, DecisionTreeClassifier>::ClassifyStepwise(
              indexFileName, prediction_oos, labels_oos, true, false, true, folderName);

  std::cout << prefixStr << " OOS : (error, precision, recall, F1, imbalance) : ("
            << error_OOS.value_or(-1.) << ", " << precision_OOS.value_or(-1.) << ", "
            << recall_OOS.value_or(-1.) << ", " << F1_OOS.value_or(-1.) << ", "
            << imbalance_OOS.value_or(-1.) << ")" << std::endl;
  std::cout << prefixStr << " IS :  (error, precision, recall, F1, imbalance) : ("
            << error_IS.value_or(-1.) << ", " << precision_IS.value_or(-1.) << ", "
            << recall_IS.value_or(-1.) << ", " << F1_IS.value_or(-1.) << ", "
