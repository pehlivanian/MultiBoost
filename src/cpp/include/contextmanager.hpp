...
private:
  template <typename ClassifierType>
  static auto computeChildPartitionInfo(const CompositeClassifier<ClassifierType>&)
      -> childPartitionInfo;
  ...
  template <typename RegressorType>
  static auto computeChildPartitionInfo(const CompositeRegressor<RegressorType>&)
      -> childPartitionInfo;
