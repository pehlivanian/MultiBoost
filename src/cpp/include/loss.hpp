...
class LossUtils {
public:
  static std::enable_if<_is_double<DataType>::value, Eigen::VectorXd> static_cast_eigen(
      const arma_dvec_type& rhs) {
    Eigen::VectorXd lhs =
        Eigen::Map<Eigen::VectorXd>(const_cast<double*>(rhs.memptr()), rhs.n_cols);
    return lhs;
  }

  static std::enable_if<!_is_double<DataType>::value, Eigen::VectorXf> static_cast_eigen(
      const arma_fvec_type& rhs) {
    Eigen::VectorXf lhs = Eigen::Map<Eigen::VectorXf>(const_cast<float*>(rhs.memptr()), rhs.n_cols);
  }

  static std::enable_if<_is_double<DataType>::value, arma_dvec_type> static_cast_arma(
      const Eigen::VectorXd& rhs) {
    arma_dvec_type lhs = arma_dvec_type{const_cast<double*>(rhs.data()), rhs.size(), false, false};
    return lhs;
  }

  static std::enable_if<!_is_double<DataType>::value, arma_fvec_type> static_cast_arma(
