...
inline std::string resolve_data_path(std::string_view filename) {
  ...
  if (s3_config_file && std::filesystem::exists(s3_config_file)) {
    ...
    if (dataset_base_name.length() >= 6 && dataset_base_name.substr(dataset_base_name.length() - 6) == "_X.csv") {
      dataset_base_name = dataset_base_name.substr(0, dataset_base_name.length() - 6);
    } else if (dataset_base_name.length() >= 6 && dataset_base_name.substr(dataset_base_name.length() - 6) == "_y.csv") {
      dataset_base_name = dataset_base_name.substr(0, dataset_base_name.length() - 6);
    }

    // Check if this is the first file of a dataset pair (X file)
    if (filename_str.length() >= 6 && filename_str.substr(filename_str.length() - 6) == "_X.csv") {
    ...
    } else if (filename_str.length() >= 6 && filename_str.substr(filename_str.length() - 6) == "_y.csv") {
