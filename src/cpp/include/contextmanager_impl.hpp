...
void ContextManager::contextInit(CompositeRegressor<RegressorType>& r, const Context& context) {
...
void ContextManager::contextInit(CompositeClassifier<ClassifierType>& c, const Context& context) {
...
template <typename RegressorType>
void ContextManager::childContext(Context& context, const CompositeRegressor<RegressorType>& r) {
  auto [partitionSize, stepSize, learningRate, activePartitionRatio] = computeChildPartitionInfo(r);
  auto [maxDepth, minLeafSize, minimumGainSplit] = computeChildModelInfo(r);
  ...
  context.childPartitionSize =
      std::vector<std::size_t>(r.childPartitionSize_.cbegin() + ind, r.childPartitionSize_.cend());
  context.childNumSteps =
      std::vector<std::size_t>(r.childNumSteps_.cbegin() + ind, r.childNumSteps_.cend());
  context.childLearningRate =
      std::vector<double>(r.childLearningRate_.cbegin() + ind, r.childLearningRate_.cend());
  context.childActivePartitionRatio = std::vector<double>(
      r.childActivePartitionRatio_.cbegin() + ind, r.childActivePartitionRatio_.cend());

  // Model args
  context.childMinLeafSize =
      std::vector<std::size_t>(r.childMinLeafSize_.cbegin() + ind, r.childMinLeafSize_.cend());
  context.childMaxDepth =
      std::vector<std::size_t>(r.childMaxDepth_.cbegin() + ind, r.childMaxDepth_.cend());
  context.childMinimumGainSplit =
      std::vector<double>(r.childMinimumGainSplit_.cbegin() + ind, r.childMinimumGainSplit_.cend());
...
template <typename ClassifierType>
void ContextManager::childContext(Context& context, const CompositeClassifier<ClassifierType>& c) {
  auto [partitionSize, stepSize, learningRate, activePartitionRatio] = computeChildPartitionInfo(c);
  auto [maxDepth, minLeafSize, minimumGainSplit] = computeChildModelInfo(c);
  ...
  context.childPartitionSize =
      std::vector<std::size_t>(c.childPartitionSize_.cbegin() + ind, c.childPartitionSize_.cend());
  context.childNumSteps =
      std::vector<std::size_t>(c.childNumSteps_.cbegin() + ind, c.childNumSteps_.cend());
  context.childLearningRate =
      std::vector<double>(c.childLearningRate_.cbegin() + ind, c.childLearningRate_.cend());
  context.childActivePartitionRatio = std::vector<double>(
      c.childActivePartitionRatio_.cbegin() + ind, c.childActivePartitionRatio_.cend());
  // Model args
  context.childMinLeafSize =
      std::vector<std::size_t>(c.childMinLeafSize_.cbegin() + ind, c.childMinLeafSize_.cend());
  context.childMaxDepth =
      std::vector<std::size_t>(c.childMaxDepth_.cbegin() + ind, c.childMaxDepth_.cend());
  context.childMinimumGainSplit =
      std::vector<double>(c.childMinimumGainSplit_.cbegin() + ind, c.childMinimumGainSplit_.cend());

  context.depth = c.depth_ + 1;
}

template <typename ClassifierType>
auto ContextManager::computeChildPartitionInfo(const CompositeClassifier<ClassifierType>& c)
    -> childPartitionInfo {
  return std::make_tuple(
      c.childPartitionSize_[1],
      c.childNumSteps_[1],
...
template <typename ClassifierType>
auto ContextManager::computeChildModelInfo(const CompositeClassifier<ClassifierType>& c)
    -> childModelInfo {
  return std::make_tuple(c.childMaxDepth_[1], c.childMinLeafSize_[1], c.childMinimumGainSplit_[1]);
}

template <typename RegressorType>
auto ContextManager::computeChildPartitionInfo(const CompositeRegressor<RegressorType>& r)
    -> childPartitionInfo {
  return std::make_tuple(
      r.childPartitionSize_[1],
      r.childNumSteps_[1],
...
template <typename RegressorType>
auto ContextManager::computeChildModelInfo(const CompositeRegressor<RegressorType>& r)
    -> childModelInfo {
  return std::make_tuple(r.childMaxDepth_[1], r.childMinLeafSize_[1], r.childMinimumGainSplit_[1]);
}
