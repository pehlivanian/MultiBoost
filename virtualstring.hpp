...
public:
  ...
  inline FixedSizeString(const FixedSizeString &rhs) noexcept : VirtualString(buffer_) { *this = rhs; }
  inline FixedSizeString(const VirtualString &rhs) noexcept : VirtualString(buffer_) { *this = rhs; }
...
namespace std {
  ...
  struct hash<typename VirtualString> {
    inline size_t operator()(const VirutalString& key) const noexcept {
      return (key.hash());
    }
  }
};
